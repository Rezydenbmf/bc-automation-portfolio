...
<!-- TODO: add a screenshot of the BC Automation GUI and/or a redacted terminal/CSV
     screenshot from a BC Follow Bot run. None currently exist in this repo. -->
...
This opens the desktop GUI launcher, which drives the registration → profile → company setup flow using the inputs in `examples/` (or your own input package generated by `src/input_package_generator/`).
...
Both tools are under active development.
